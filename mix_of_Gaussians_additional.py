...
min_iter = 2 * 10**3  # Total number of iterations is at least this.
max_iter = min_iter # Maximum number of iterations before giving up on coupling
S = 128  # Number of independent unbiased estimators
...
cppyy.cppdef("""
void log_likelihood_cpp(double* vec_out, double* x, double* y, int D, int N) {
    /*
...
def simul(D, N, coupled=True):
    """
    `D` is the dimensionality of the state space.
    `N` is the number of SMC particles.
    """
    print("Working on D =", D, "N =", N)


    # Simulation as per Section B.2 of Middleton et al. (2019)
    x_star = np.array([-3.0, 0.0, 3.0, 6.0])[:D]

    """
    The cluster allocation in the next line is not drawn randomly according to
    the model but instead follows what was done in Middleton et al. (2019) per
    ...
    def log_likelihood(x_i):
        ...
        ret = np.empty(N)
        log_likelihood_cpp(ret, x_arr, y, D, N)
        return ret
    ...
    def MCMC_update(x_i, rng, α=1.0):
        ...
        Metropolis-Hastings algorithm with as proposal a Gaussian centered at
        `x` with identity covariance. Thus, the proposal is a symmetric random
        walk.
        """
        N = len(x_i)
        x_arr = np.stack(x_i)
    ...
    def uncoupled_MCMC_update(z_i, rng, α=1.0):
        for j in range(2):
            z_i[:, j] = MCMC_update(
                x_i=z_i[:, j], rng=rng, α=α
            )
        
        for i in range(z_i.shape[0]):
            if np.allclose(z_i[i, 0], z_i[i, 1]):
                z_i[i, 1] = z_i[i, 0]
    ...
    setup = cpmcmc.adapt_SMC(
        10**4, sample_from_prior, log_likelihood, MCMC_update, test_func,
        rng=rng
    )
    ...
    result_list = [cpmcmc.repeat_cMCMC(
        ...
        MCMC_update, setup,
        coupled_MCMC_update if coupled else uncoupled_MCMC_update, h, rng=rng,
        PIMH_prob=prob
    ) for prob in [0.0, 0.05, 0.5, 1.0]]
    ...
    def plot_result(
        result, ax, n_l=10, prob=None, label=None, color="black",
        h_est_list=None
    ):
        ...
        for ind, l in enumerate(l_list):
            if compute_h:
                ...
                for k in range(l):
                    for s in range(S):
                        BC_arr[s, k] = np.sum(np.minimum(l - k, np.arange(
                            k + 1, result["meeting time"][s]
                        ) - k) * h_diff[s, (k + 1):result["meeting time"][s]])
    ...
    axs[ind_tau, 0].set_ylim((-0.2, np.log10(max_iter) + 0.2))
    axs[2, 0].set_ylim((7.0 * 10**-6, 10**9))
    ...
    fig.savefig("mix_of_Guassians_D" + str(D) + "N" + str(N) + (
        "" if coupled else "_uncoupled"
    ) + ".pdf")
